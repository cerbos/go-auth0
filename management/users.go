...
func (u *UserIdentitySchema) UnmarshalJSON(data []byte) error {
	...
	var workaround struct {
		*unmarshaler
		RawUserID any `json:"user_id,omitempty"`
	}

	workaround.unmarshaler = &value

	if err := json.Unmarshal(data, &workaround); err != nil {
		return err
	}

	var userID string
	switch rawUserID := workaround.RawUserID.(type) {
	case string:
		userID = rawUserID
	case float64:
		userID = strconv.FormatInt(int64(rawUserID), 10)
	}
